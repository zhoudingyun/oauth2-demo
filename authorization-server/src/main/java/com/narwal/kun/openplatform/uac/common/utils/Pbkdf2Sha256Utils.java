package com.narwal.kun.openplatform.uac.common.utils;
...
public class Pbkdf2Sha256Utils {
    ...
    //算法名称
    private static final String algorithm = "mypbkdf2_sha256";
     ...
     * @return
     */
    public static String encode(String password, String salt, int iterations, String algorithm) {
        // returns hashed password, along with algorithm, number of iterations and salt
        String hash = getEncodedHash(password, salt, iterations);
        return String.format("%s$%d$%s$%s", algorithm, iterations, salt, hash);
    }
    ...
    public static boolean verification(String password, String hashedPassword) {
        ...
        String salt = parts[2];
        String hash = encode(password, salt, iterations, parts[0]);
